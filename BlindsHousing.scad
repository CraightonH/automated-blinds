...
difference() {
  ...
  color("blue") translate([0,0,3]) 
    cylinder(r=14.1, h=cylinder_cutout_depth, $fn=100, center=true);
  
  // Red: top square cutout for motor
  color("red") translate([5.7,0,6]) 
    cube([17,18,30.5], center=true);
  
  // Black: top motor channel
  color("black") translate([10,0,6]) 
    cube([17,15,30.5], center=true);
  
  // Purple: cylinder holes for motor mounts
  color("purple") translate([0,17.4,10]) 
    cylinder(r=3.35, h=2, $fn=100);
  color("purple") translate([0,-17.4,10]) 
    cylinder(r=3.35, h=2, $fn=100);
  
  // Orange: square cutouts for motor mounts
  color("orange") translate([0,14.05,11]) 
    cube([6.7,6.7,2], center=true);
  color("orange") translate([0,-14.05,11]) 
    cube([6.7,6.7,2], center=true);
  
  // Green: screw holes
  color("green") translate([0,-17.4,6]) 
    cylinder(r=1.5, h=5, $fn=100);
  color("green") translate([0,17.4,6]) 
    cylinder(r=1.5, h=5, $fn=100);
  
  // White: top channel for wires
  color("white") translate([14,-(7.5),-11]) 
    cube([4,15,block_z]);
...
difference() {
  ...
  color("white") 
    translate([14,-(7.5),-(equipment_housing_calc_length + block_z/ 2)]) 
      cube([4,15,equipment_housing_calc_length+2]);
}
